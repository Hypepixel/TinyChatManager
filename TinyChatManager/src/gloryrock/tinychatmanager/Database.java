...
public class Database
{
    ...
    Database()
    {
        ...
        this.tablePrefix = FileManager.getConfig().getFileData()
            .getString(ConfigData.Values.SQL_TABLE_PREFIX.toString());
    ...
    private void connect()
    {
        synchronized (this)
        {
            ...
            catch (SQLException exception)
            {
                ...
                Messages.log(
                    "§cPlease check if the sql server is running and you entered the right username and password.");
            }
    ...
    {
        this.update(
            "CREATE TABLE IF NOT EXISTS `%p%users` (`uuid` CHAR(36) NOT NULL, `group` VARCHAR(64) NULL DEFAULT NULL, `force_group` BOOLEAN NULL DEFAULT NULL, `subgroup` VARCHAR(64) NULL DEFAULT NULL, `chat_color` CHAR(2) NULL DEFAULT NULL, `chat_formatting` CHAR(2) NULL DEFAULT NULL, PRIMARY KEY(`uuid`))ENGINE = InnoDB CHARSET = utf8 COLLATE utf8_bin;");
        this.update(
            "CREATE TABLE IF NOT EXISTS `%p%groups` (`group` VARCHAR(64) not null, UNIQUE(`group`), prefix VARCHAR(128) default NULL null, suffix VARCHAR(128) default NULL null, chat_color CHAR(2) default NULL null, chat_formatting CHAR(2) default NULL null, join_msg VARCHAR(255) default NULL null, quit_msg VARCHAR(255) default NULL null)ENGINE = InnoDB CHARSET = utf8 COLLATE utf8_bin;");
        this.update(
            "CREATE TABLE IF NOT EXISTS `%p%subgroups` ( `group` VARCHAR(64) NOT NULL , UNIQUE(`group`), `prefix` VARCHAR(128) default NULL null , `suffix` VARCHAR(128) default NULL null ) ENGINE = InnoDB CHARSET = utf8 COLLATE utf8_bin;");
    }
    ...
    private void uploadGroups() throws SQLException
    {
        ...
        for (final String groupName : groups)
        {
            ...
            final String sql = "UPDATE `" + this.getTablePrefix()
                + "groups` SET `prefix`= ?,`suffix`= ?,`chat_color`= ?,`chat_formatting`= ?,`join_msg`= ?,`quit_msg`= ? WHERE `group` = ?";
            final PreparedStatement stmt = this.prepareStatement(sql);
        ...
        for (final String groupName : groups)
        {
            try
            {
                final PreparedStatement stmt = this
                    .prepareStatement("INSERT INTO `" + this.getTablePrefix() + "subgroups`(`group`) VALUES (?)");
                stmt.setString(1, groupName);
                stmt.executeUpdate();
            ...
            final String sql = "UPDATE `" + this.getTablePrefix()
                + "subgroups` SET `prefix`= ?,`suffix`= ? WHERE `group` = ?";
        ...
        if (listOfFiles != null)
        {
            for (final File listOfFile : listOfFiles)
            {
                if (listOfFile.isFile())
                {
                    ...
                    final String sql = "INSERT INTO `" + this.getTablePrefix()
                        + "users`(`uuid`, `group`, `force_group`, `subgroup`, `chat_color`, `chat_formatting`) VALUES (?,?,?,?,?,?,?)";
                    final PreparedStatement stmt = this.prepareStatement(sql);
